...
Importable as a library (used by nvme-failure-injection-harness):

    from explore import snapshot, compute_diff, evaluate, Thresholds
    before = snapshot()                       # dict: meta + log_pages + smart_normalised
    ... do something to the device ...
    after = snapshot()
    changes = compute_diff(before, after)     # list[Change]
    findings = evaluate(after, Thresholds())  # list[Finding]
...
__all__ = [
    "snapshot", "compute_diff", "evaluate", "Change", "Finding", "Thresholds",
    "Device", "discover", "NvmeError", "COLLECTORS",
    "decode_smart_log_page", "decode_critical_warning", "normalise_smart",
]
...
def snapshot(controller: str | None = None, namespace: str | None = None,
             sections: Iterable[str] | None = None) -> dict:
    """
    Library entry point: capture Identify + log pages from a live device and
    return the snapshot dict (meta / log_pages / smart_normalised). Section names
    are the keys of COLLECTORS; None means all of them.
    """
    dev = discover(controller, namespace)
    return build_snapshot(dev, list(sections) if sections else list(COLLECTORS))
...
class Change:
    ...
    def as_dict(self) -> dict:
        return {"path": self.path, "before": self.before,
...
def compute_diff(before: dict, after: dict) -> list[Change]:
    """
    Field-level diff of two snapshots (or two log_pages dicts). Metadata and
    timestamps are ignored. Returns the changed fields, sorted by path.
    """
    fa = _flatten(before.get("log_pages", before))
    fb = _flatten(after.get("log_pages", after))
    out: list[Change] = []
...
def cmd_diff(args: argparse.Namespace) -> int:
    ...
    if not changes:
        ...
        return 0
    print(f"{len(changes)} field(s) changed  ({args.before} -> {args.after})\n")
    for c in changes:
        delta = f"   (Δ {c.delta:+})" if c.delta is not None else ""
        print(f"  {c.path}\n      {c.before}  ->  {c.after}{delta}")
    return 0
...
def evaluate(snap: dict, t: Thresholds | None = None) -> list[Finding]:
    t = t or Thresholds()
